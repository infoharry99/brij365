...
<section class="mbx-screen <?php echo e($selected ? 'has-msg' : ''); ?>" aria-label="Mailbox">
  ...
  <aside class="mbx-rail">
      ...
      <summary class="mbx-acct-sum">
        ...
        <div class="mbx-ai">
          ...
          <small style="display:inline-flex; align-items:center; gap:4px;">
            <?php echo e($mailboxAccount->email); ?>

            <button type="button" onclick="copyEmailToClipboard('<?php echo e($mailboxAccount->email); ?>', event)" title="Copy email address" style="background:none; border:none; color:var(--c-muted); cursor:pointer; padding:1px 4px; font-size:11px; border-radius:3px;" onmouseover="this.style.color='var(--c-accent)'" onmouseout="this.style.color='var(--c-muted)'">
        ...
        <?php $__currentLoopData = $availableAccounts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $acct): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <a href="<?php echo e(route('mailbox.external.show', $acct)); ?>"
            ...
            <div class="mbx-ai">
              ...
              <small style="display:inline-flex; align-items:center; gap:4px;">
                <?php echo e($acct->email); ?>

                <button type="button" onclick="copyEmailToClipboard('<?php echo e($acct->email); ?>', event)" title="Copy email address" style="background:none; border:none; color:var(--c-muted); cursor:pointer; padding:1px 4px; font-size:11px; border-radius:3px;" onmouseover="this.style.color='var(--c-accent)'" onmouseout="this.style.color='var(--c-muted)'">
                  <i class="fa-regular fa-copy" aria-hidden="true"></i>
                </button>
              </small>
            </div>
    ...
    <div class="mbx-sync" id="mbxSyncContainer" data-sync-url="<?php echo e(route('mailbox.accounts.sync-json', $mailboxAccount)); ?>">
      <span class="mbx-sdot <?php echo e($mailboxAccount->status === 'active' ? 'on' : ''); ?>" id="mbxSyncDot"></span>
      ...
      <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('update', $mailboxAccount)): ?>
        <form id="mbxSyncForm" method="POST" action="<?php echo e(route('mailbox.accounts.sync', $mailboxAccount)); ?>">
          <?php echo csrf_field(); ?>
  ...
  <section class="mbx-list">
    ...
    <?php $isTrashView = ($folder?->special_use === 'trash'); ?>
    <div id="bulkActionBar" style="display:none; align-items:center; justify-content:space-between; background:#FFF7ED; border:1px solid #FFEDD5; padding:8px 14px; margin:6px 10px; border-radius:8px; font-size:12.5px; color:#9A3412; font-weight:600;">
      ...
      <div style="display:flex; align-items:center; gap:6px;">
        <button type="button" class="mbx-btn danger" onclick="openBulkDeleteDialog(<?php echo e($isTrashView ? 'true' : 'false'); ?>)" style="padding:4px 10px; font-size:11.5px;">
          <i class="fa-solid <?php echo e($isTrashView ? 'fa-trash-can' : 'fa-trash'); ?>" aria-hidden="true"></i>
          <?php echo e($isTrashView ? 'Delete permanently' : 'Delete selected'); ?>
          ...
          <span class="mbx-rcopy">
            <span class="mbx-rtop">
              <strong class="mbx-rfrom">
                ...
                <?php if(!$isOutboxMsg && ($email->thread_count ?? 1) > 1): ?>
                  <span class="mbx-lcnt" style="font-size:10px; padding:1px 6px; margin-left:4px;" title="<?php echo e($email->thread_count); ?> messages in thread"><?php echo e($email->thread_count); ?></span>
                <?php endif; ?>
                <?php if($statePill): ?>
    ...
    <?php if($selected): ?>
      ...
      <div class="mbx-thread" role="list" aria-label="Message thread" data-sort-order="asc">
        ...
        <?php $__currentLoopData = $threadMessages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $msg): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          ...
          <article class="mbx-card <?php echo e($msg->id === $selected->id ? 'cur' : ''); ?> <?php echo e($isExpanded ? 'is-expanded' : 'is-collapsed'); ?>"
                   role="listitem"
            ...
            <?php if($msg->attachments?->isNotEmpty()): ?>
              <div class="mbx-ca" style="margin-top:14px; padding-top:10px; border-top:1px solid var(--c-border, #E2E8F0);">
                <strong style="display:block; font-size:12.5px; color:var(--c-muted, #64748B); margin-bottom:8px;">
                  <i class="fa-solid fa-paperclip" aria-hidden="true"></i> Attachments (<?php echo e($msg->attachments->count()); ?>)
                </strong>
                  ...
                  <?php $__currentLoopData = $msg->attachments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $att): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    <div class="mbx-att-card" style="display:inline-flex; align-items:center; gap:8px; background:#F8FAFC; border:1px solid #E2E8F0; border-radius:8px; padding:6px 10px; font-size:12.5px; box-shadow:0 1px 2px rgba(0,0,0,0.03);">
                      ...
                      <span style="display:flex; flex-direction:column; line-height:1.2;">
                        <strong style="color:#0F172A; max-width:170px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="<?php echo e($fileName); ?>"><?php echo e($fileName); ?></strong>
                        <small style="color:#64748B; font-size:11px;"><?php echo e($sizeKb); ?> KB</small>
                      </span>
                      <div style="display:flex; align-items:center; gap:4px; margin-left:6px;">
                        <button
                          ...
                          onclick="openMailAttachmentPreview('<?php echo e(addslashes($fileName)); ?>', '<?php echo e($previewUrl); ?>', '<?php echo e($downloadUrl); ?>', '<?php echo e($att->mime_type); ?>', '<?php echo e($sizeKb); ?> KB')"
                          title="Preview file"
                        >
      ...
      <footer class="mbx-af">
        ...
        <?php if(!($isOutboxFolder ?? false)): ?>
          ...
          <?php $__currentLoopData = ['archive' => ['Archive','fa-box-archive'], 'spam' => ['Spam','fa-shield-halved']]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $act => [$lbl, $icon]): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          ...
          <?php $isTrashView = ($folder?->special_use === 'trash'); ?>
          <button type="button" class="mbx-btn danger" onclick="openDeleteDialog(<?php echo e($isTrashView ? 'true' : 'false'); ?>)">
            <i class="fa-solid <?php echo e($isTrashView ? 'fa-trash-can' : 'fa-trash'); ?>" aria-hidden="true"></i>
            <?php echo e($isTrashView ? 'Delete permanently' : 'Delete'); ?>
...
<div id="deleteModal" style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(15,23,42,0.55); backdrop-filter:blur(3px); z-index:99999; align-items:center; justify-content:center;">
  <div style="background:#fff; border-radius:14px; width:92%; max-width:440px; padding:24px; box-shadow:0 20px 25px -5px rgba(0,0,0,0.15); font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
    ...
    <div id="standardDeleteOptions" style="display:flex; flex-direction:column; gap:10px;">
      <?php if($selected): ?>
        <form id="singleTrashForm" method="POST" action="<?php echo e(route('mailbox.external.state', $selected)); ?>">
          <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
        ...
        <form id="singleArchiveForm" method="POST" action="<?php echo e(route('mailbox.external.state', $selected)); ?>">
          <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
    ...
    <div id="permanentDeleteOptions" style="display:none; flex-direction:column; gap:10px;">
      <?php if($selected): ?>
        <form id="singlePermDeleteForm" method="POST" action="<?php echo e(route('mailbox.external.state', $selected)); ?>">
          <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
...
<script>
  ...
  function submitBulkAction(action) {
    ...
    form.method = 'POST';
    form.action = '<?php echo e(route("mailbox.accounts.bulk-state", $mailboxAccount)); ?>';
...
<?php echo $__env->make('layouts.builder360-classic', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/developer/public_html/build365/resources/views/mailbox/external/show.blade.php ENDPATH**/ ?>