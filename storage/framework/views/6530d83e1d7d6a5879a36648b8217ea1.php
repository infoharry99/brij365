                            ...
                            <details class="b360-collab-create b360-group-manage-details" x-data="{ groupTab: 'members' }">
                                ...
                                <div class="b360-collab-popover b360-chat-create-panel" role="dialog" aria-modal="true" aria-labelledby="manage-group-title" style="width: min(580px, calc(100vw - 32px)); max-height: min(680px, calc(100vh - 40px)); overflow: hidden; display: flex; flex-direction: column;">
                                    ...
                                    <div class="b360-popover-head" style="padding: 18px 24px; border-bottom: 1px solid #E2E8F2; flex-shrink: 0; background: #fff;">
                                        <div>
                                            <h2 id="manage-group-title" style="font-size: 18px; font-weight: 700; color: #0F172A; margin: 0;"><?php echo e($selectedDisplayTitle); ?></h2>
                                            <p style="font-size: 12px; color: #64748B; margin: 3px 0 0;"><?php echo e(str($selectedConversation->type)->headline()); ?> · Manage settings & members</p>
                                        </div>
                                    ...
                                    <div style="flex: 1; overflow-y: auto; padding: 20px 24px; display: flex; flex-direction: column; gap: 16px; box-sizing: border-box;">
                                        ...
                                        <nav style="display: flex; gap: 8px; border-bottom: 1px solid #E2E8F2; padding-bottom: 12px; flex-shrink: 0;" aria-label="Group tabs">
                                            ...
                                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('manageMembers', $selectedConversation)): ?>
                                                <?php if($selectedConversation->type !== 'direct_message'): ?>
                                                    <button
                                                        type="button"
                                                        class="cc-rail-tab"
                                                    ...
                                                    <?php if($member->user): ?>
                                                        <div style="display: flex; align-items: center; justify-content: space-between; padding: 10px 14px; border-bottom: 1px solid #F1F5F9; gap: 12px;">
                                                            <div style="display: flex; align-items: center; gap: 10px; min-width: 0; flex: 1;">
                                                                <?php if (isset($component)) { $__componentOriginal2252ef3298868bc9de4c534a2a83a2a2 = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal2252ef3298868bc9de4c534a2a83a2a2 = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.ui.user-avatar','data' => ['user' => $member->user,'label' => $member->user->name,'style' => 'width: 34px; height: 34px; font-size: 12px; flex-shrink: 0;']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
<?php $component->withName('ui.user-avatar'); ?>
<?php if ($component->shouldRender()): ?>
<?php $__env->startComponent($component->resolveView(), $component->data()); ?>
<?php if (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag): ?>
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['user' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($member->user),'label' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($member->user->name),'style' => 'width: 34px; height: 34px; font-size: 12px; flex-shrink: 0;']); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
<?php if (isset($__attributesOriginal2252ef3298868bc9de4c534a2a83a2a2)): ?>
<?php $attributes = $__attributesOriginal2252ef3298868bc9de4c534a2a83a2a2; ?>
...
<?php endif; ?>
                                                                <div style="min-width: 0; flex: 1;">
                                                                    <strong style="display: flex; align-items: center; gap: 6px; font-size: 13px; font-weight: 600; color: #0F172A; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                                                        ...
                                                                        <?php if((int)$selectedConversation->owner_user_id === (int)$member->user_id): ?>
                                                                        ...
                                                                        <?php endif; ?>
                                                                        <?php if((int)$member->user_id === (int)auth()->id()): ?>
                                                                    ...
                                                                    <small style="display: block; color: #64748B; font-size: 11px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><?php echo e($member->user->role?->name ?? $member->user->email); ?></small>
                                                                </div>
                                                            </div>
                                                            <?php if($selectedConversation->type !== 'direct_message'): ?>
                                        ...
                                        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('manageMembers', $selectedConversation)): ?>
                                            <?php if($selectedConversation->type !== 'direct_message'): ?>
                                                <div x-show="groupTab === 'add'" x-cloak x-data="peopleFilter" style="display: flex; flex-direction: column; gap: 12px;">
                                                    <form method="POST" action="<?php echo e(route('collaboration.chat.conversations.members.store', $selectedConversation)); ?>" style="display: flex; flex-direction: column; gap: 12px;">
                                                        <?php echo csrf_field(); ?>
                                                        <?php
                                                        ...
                                                        <div class="b360-chat-member-picker" style="max-height: 240px; overflow-y: auto; border: 1px solid #E2E8F2; border-radius: 12px; background: #fff;">
                                                            <?php $__currentLoopData = $availableAddUsers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                                <label class="b360-chat-member-option" data-people-option data-search="<?php echo e(str($user->name.' '.$user->email.' '.($user->role?->name ?? ''))->lower()); ?>">
                                                                    <input type="checkbox" name="member_user_ids[]" value="<?php echo e($user->id); ?>" x-on:change="togglePerson">
                                                                    <span>
                                                                        <?php echo e($user->name); ?>
                                                            ...
                                                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                                            <?php if($availableAddUsers->isEmpty()): ?>
                                                ...
                                                <div x-show="groupTab === 'edit'" x-cloak style="display: flex; flex-direction: column; gap: 12px;">
                                                    <form method="POST" action="<?php echo e(route('collaboration.chat.conversations.update', $selectedConversation)); ?>" class="b360-chat-form" style="gap: 12px;">
                                                        <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                                                        <label style="display: block;">
                                                            <span>Group / Channel title *</span>
                                                            <input type="text" name="title" maxlength="160" required value="<?php echo e(old('title', $selectedConversation->title)); ?>">
                                                        </label>
                                                        <label style="display: block;">
                                                            <span>Description / Purpose <small>(optional)</small></span>
                                                            <textarea name="description" maxlength="500" placeholder="Purpose of this group or channel" style="min-height: 80px;"><?php echo e(old('description', $selectedConversation->description)); ?></textarea>
                                                        </label>
                                                        <div style="display: flex; justify-content: flex-end; padding-top: 4px;">
                            ...
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('archive', $selectedConversation)): ?>
                                <form method="POST" action="<?php echo e(route('collaboration.chat.conversations.archive', $selectedConversation)); ?>"><?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?><button type="submit" title="Archive conversation" aria-label="Archive conversation"><i class="fa-solid fa-box-archive"></i></button>
                            </form>
                            <?php endif; ?>
                        <?php endif; ?>
                    </div>
