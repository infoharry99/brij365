...
class PurchaseRequisitionIndexRequest extends FormRequest
{
    public function authorize(): bool
    {
        return $this->user()?->can('viewAny', PurchaseRequisition::class) === true;
    }
    ...
    {
        return [
            'status' => ['nullable', 'string', Rule::in(['submitted', 'approved', 'rejected', 'cancelled'])],
        ...
        return [
            function (Validator $validator): void {
                app(QueryFilterPolicy::class)->rejectUnexpected(
                    $validator,
                    $this->query(),
                    ['status', 'project_id', 'per_page', 'page'],
                );
