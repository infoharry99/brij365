...
class ProjectApprovalIndexRequest extends FormRequest
{
    public function authorize(): bool
    {
        return $this->user()?->can('viewAny', ProjectApproval::class) === true;
    }
    ...
    public function rules(): array
    {
        return [
            ...
            'status' => ['nullable', 'string', Rule::in(['applied', 'approved', 'verified'])],
            'approval_type' => ['nullable', 'string', 'max:120'],
            'expires_within_days' => ['nullable', 'integer', 'min:0', 'max:3650'],
        ...
        return [
            function (Validator $validator): void {
                app(QueryFilterPolicy::class)->rejectUnexpected(
                    $validator,
                    $this->query(),
                    ['project_id', 'status', 'approval_type', 'expires_within_days', 'per_page', 'page'],
                );
